...
if (fs.existsSync('./server.key') && fs.existsSync('./server.crt')) {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`⚠️ Para HTTPS, genere certificados con:`);
    console.log(`   openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout server.key -out server.crt`);
  });
}
